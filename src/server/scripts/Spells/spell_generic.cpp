...
class spell_gen_tricky_treat : public SpellScriptLoader
{
    public:
        ...
        class spell_gen_tricky_treat_SpellScript : public SpellScript
        {
            ...
            void Register()
            {
                OnEffectHit += SpellEffectFn(spell_gen_tricky_treat_SpellScript::HandleScript, EFFECT_0, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };
                ...
                if (Unit* rider = GetCaster()->GetCharmer())
                {
                    if (Player* target = GetHitPlayer())
                    {
                        if (target->HasAura(SPELL_ON_TOURNAMENT_MOUNT) && target->GetVehicleBase())
                            rider->CastSpell(target, SPELL_MOUNTED_DUEL, true);
                    }
                    else if (Unit* target = GetHitUnit())
                    {
                        if (target->GetCharmer() && target->GetCharmer()->GetTypeId() == TYPEID_PLAYER && target->GetCharmer()->HasAura(SPELL_ON_TOURNAMENT_MOUNT))
                            rider->CastSpell(target->GetCharmer(), SPELL_MOUNTED_DUEL, true);
                    }
                }
            }
...
class spell_gen_elunes_blessing : public SpellScriptLoader
{
    public:
        spell_gen_elunes_blessing() : SpellScriptLoader("spell_gen_elunes_blessing") { }

        class spell_gen_elunes_blessing_SpellScript : public SpellScript
        {
            PrepareSpellScript(spell_gen_elunes_blessing_SpellScript);
            ...
            void HandleScriptEffect(SpellEffIndex /*effIndex*/)
            {
                ...
                if (target->GetQuestStatus(8868) == QUEST_STATUS_INCOMPLETE)
                    target->CastSpell(target, SPELL_ELUNE_QUEST_CREDIT, true);
            }

            void Register()
            {
                OnEffectHitTarget += SpellEffectFn(spell_gen_elunes_blessing_SpellScript::HandleScriptEffect, EFFECT_1, SPELL_EFFECT_SCRIPT_EFFECT);
            }
        };

        SpellScript* GetSpellScript() const
        {
            return new spell_gen_elunes_blessing_SpellScript();
        }
};
