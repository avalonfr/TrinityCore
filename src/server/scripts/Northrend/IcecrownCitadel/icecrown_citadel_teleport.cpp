...
class icecrown_citadel_teleport : public GameObjectScript
{
    public:
        ...
        {
            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Téléportation vers Marteau de Lumiere", GOSSIP_SENDER_ICC_PORT, LIGHT_S_HAMMER_TELEPORT);
            if (InstanceScript* instance = go->GetInstanceScript())
            {
                ...
                if (instance->GetBossState(DATA_VALITHRIA_DREAMWALKER) == DONE)
                    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Téléportation au repaire de Sindragosa", GOSSIP_SENDER_ICC_PORT, SINDRAGOSA_S_LAIR_TELEPORT);
            }
