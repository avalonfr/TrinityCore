...
bool OutdoorPvPWG::SetupOutdoorPvP()
{
    ...
    if (!result)
        sLog->outError(LOG_FILTER_BATTLEFIELD,"Cannot find siege workshop master or spirit guides in creature!");
    else
    {
        do
        {
    ...
    do
    {
        ...
        if (capturePointEntry)
        {
            ...
            if (!engGuid)
            {
                sLog->outError(LOG_FILTER_BATTLEFIELD,"Cannot find nearby siege workshop master!");
                continue;
            }
            else
            ...
            {
                delete workshop;
                sLog->outError(LOG_FILTER_BATTLEFIELD,"Cannot add capture point!");
                continue;
            }
    ...
    if (!m_gate)
    {
        sLog->outError(LOG_FILTER_BATTLEFIELD,"Cannot find wintergrasp fortress gate!");
        return false;
    }
...
void OutdoorPvPWG::ModifyWorkshopCount(TeamId team, bool add)
{
    ...
    if (add)
        ...
        sLog->outError(LOG_FILTER_BATTLEFIELD,"OutdoorPvPWG::ModifyWorkshopCount: negative workshop count!");
                            ...
                            if (i->getSource()->GetDistance2d(5314.51f, 2703.69f) <= 5 && i->getSource()->IsOnVehicle(i->getSource()->GetVehicleCreatureBase()) && i->getSource()->isAlive())
                            {
                                ...
                                else
                                {
                                    ...
                                    if (Driver && Driver->isAlive())
                                    {
										...
										Driver->EnterVehicle(CAST_AI(Unit,vehicle),0);
...
void OutdoorPvPWG::RewardMarkOfHonor(Player *plr, uint32 count)
{
    ...
    {
        sLog->outError(LOG_FILTER_BATTLEFIELD,"Wintergrasp reward item (Entry %u) not exist in `item_template`.", WG_MARK_OF_HONOR);
        return;
    }
...
OPvPCapturePointWG::OPvPCapturePointWG(OutdoorPvPWG *opvp, BuildingState *state) : OPvPCapturePoint(opvp), m_buildingState(state), m_wintergrasp(opvp),m_spiritguide_horde(NULL), m_engGuid(0), m_engineer(NULL), m_spiritguide(NULL), m_spiGuid(0), m_spiritguide_alliance(NULL) { }
...
class outdoorpvp_wg : public OutdoorPvPScript
{
    public:
        outdoorpvp_wg() : OutdoorPvPScript("outdoorpvp_wg") { }
